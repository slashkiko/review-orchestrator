...
v1.2 inventories package scripts (including nested packages), mise tasks, and configured secret/mutation settings at the immutable target boundary. Every inventory entry has a deterministic `gate_id` in the snapshot identity. A package uses its own `packageManager` first, then its own lockfile (priority `pnpm-lock.yaml`, `yarn.lock`, `bun.lockb`, `package-lock.json`). It may inherit an ancestor manager or lockfile only when that ancestor's `package.json` workspace list or `pnpm-workspace.yaml` `packages:` list includes it. Workspace matching is segment-aware: `*` matches one segment, `**` zero or more, and ordered `!` patterns exclude a prior match. Otherwise it defaults to npm. Makefile is deliberately excluded: parsing its complete grammar without executing untrusted repository content is not reliable enough for an execution inventory. For the exact approval and one-shot runner protocol, read [gate-runner.md](gate-runner.md).
