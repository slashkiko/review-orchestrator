...
Snapshot hashes, checkout binding, approval schema/hash/argv matching, scope-gap matching, routing selection, and artifact validation are deterministic checks. Actual user approval and its `user_exact` provenance are main-agent procedural assertions: v1.2 has no signature or external attestation. A structurally valid artifact can be manually forged; do not describe it as trusted provenance.
